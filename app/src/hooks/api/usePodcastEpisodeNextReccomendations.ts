 ...
 */
export function usePodcastEpisodeNextReccomendations(vanityID: string) {
  return useQuery({
    queryKey: ["podcast", "episode", "reccomendations", vanityID],
    queryFn: () => getPodcastEpisodeNextReccomendations(vanityID),
  });
}
 ...
 */
export async function getPodcastEpisodeNextReccomendations(vanityID: string) {
  const res = await fetch(
    `${apiBaseUrl}/v1/podcast/reccomendations/next?current_episode_vanity_id=${vanityID}&limit=10`,
  );
