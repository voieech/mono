...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  //
  // Notes on this index
  // 1. This is using "id DESC" to optimize for recent likes first, so that when
  // we filter by user, the returned results are already sorted by "id" which is
  // a UUID v7 value which is sorted by time already, without making the DB
  // retrieve all likes before sorting in memory.
  ...
  await sql`
    CREATE INDEX user_like_ordered_by_id_index_covering
