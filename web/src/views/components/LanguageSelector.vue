...
<template>
  <div
    class="flex flex-row items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-2 py-1 text-sm sm:text-base"
  >
    <svg
      width="24"
      height="24"
    ...
    <select
      ...
      @change="(e: Event) => updateLangQueryParam((e?.target as any)?.value)"
    >
      <option
        ...
        :value="locale"
      >
        {{ languageCodeToDisplayMapping[locale] ?? "error" }}
      </option>
    </select>
  </div>
</template>
